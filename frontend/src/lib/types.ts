...
export interface ServerResponse {
  type: 'connected' | 'login_success' | 'login_failed' | 'mfa_required' | 'mfa_success' | 'mfa_failed' | 'download_started' | 'download_complete' | 'types_creation_started' | 'types_creation_complete' | 'error' | 'closed' | 'log_message';
  session_id?: string;
  message?: string;
  level?: 'info' | 'warning';
  [key: string]: any;
...
export interface ClientCommand {
  command: 'start_login' | 'submit_mfa_code' | 'download_and_create_types' | 'retry_login' | 'retry_mfa' | 'close';
  username?: string;
  password?: string;
  code?: string;
  uuid?: string;
