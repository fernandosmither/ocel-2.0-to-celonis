...
import type { AppState, LogEntry, ConnectionStatus, ServerResponse, ClientCommand } from './types';
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  connect: () => {
    ...
    if (websocket?.readyState === WebSocket.OPEN || appState === 'connecting') {
      addLog({ level: 'debug', message: `Connection attempt skipped - current state: ${appState}, websocket readyState: ${websocket?.readyState}` });
      return;
    }
    
    // Close existing websocket if it exists but not open
    if (websocket && websocket.readyState !== WebSocket.OPEN) {
      addLog({ level: 'debug', message: `Closing stale websocket (readyState: ${websocket.readyState})` });
      websocket.close();
    }
    ...
    try {
      ...
      set({ websocket: ws });
      addLog({ level: 'debug', message: `Created new websocket, readyState: ${ws.readyState}` });
      ...
      ws.onopen = () => {
        ...
        if (currentState.websocket === ws) {
          ...
          addLog({ level: 'debug', message: 'Ignoring onopen for stale websocket' });
        }
      };
      ...
      ws.onmessage = (event) => {
        ...
        if (currentState.websocket === ws) {
          try {
            ...
            addLog({ level: 'error', message: 'Failed to parse server message' });
          }
        }
      };
      ...
      addLog({ level: 'error', message: 'Failed to create WebSocket connection' });
    }
  },
...
function handleServerResponse(response: ServerResponse) {
  ...
  switch (response.type) {
    case 'connected':
      ...
      addLog({ level: 'success', message: response.message || 'Connected to server' });
      break;
    ...
    case 'mfa_failed':
      setAppState('mfa_required');
      addLog({ level: 'error', message: response.message || 'MFA authentication failed' });
      break;
    ...
    case 'types_creation_complete':
      ...
      addLog({ level: 'success', message: 'Types creation completed successfully' });
      break;
      
    case 'error':
      setAppState('error');
      addLog({ level: 'error', message: response.message || 'An error occurred' });
      break;
